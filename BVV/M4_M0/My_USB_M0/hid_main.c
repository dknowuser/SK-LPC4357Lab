...
{
	//GlobalDebug[GlobalCounter] = 'E' ;
	//GlobalCounter=GlobalCounter+1;
/*	if (DebugSem == 1)
	{
		GlobalDebug[GlobalCounter] = 0x00 ;
	}
	else
	{
		GlobalDebug[GlobalCounter] = 0xF2 ;
	}
	GlobalCounter=GlobalCounter+1;	
	DebugSem=2;*/
	
	switch (event) {//EP0_patch
	case USB_EVT_OUT_NAK:
	//	GlobalDebug[GlobalCounter] = 'N' ;
	//GlobalCounter=GlobalCounter+1;
		if (g_ep0RxBusy) {
			/* we already queued the buffer so ignore this NAK event. */
		//	GlobalDebug[GlobalCounter] = 'R' ;
	//GlobalCounter=GlobalCounter+1;
			return LPC_OK;
		}
		else {
	...
	//	GlobalDebug[GlobalCounter] = 'S' ;
	//GlobalCounter=GlobalCounter+1;
	case USB_EVT_OUT:
		/* we received the packet so clear the flag. */
	//GlobalDebug[GlobalCounter] = 'O' ;
	//GlobalCounter=GlobalCounter+1;
		g_ep0RxBusy = 0;
		break;
	}
...
int main(void)
{
	...
	while (1) {
		/* Sleep until next IQ happens */
		GlobalDebug[GlobalCounter] = 0xAA ;
	  GlobalCounter=GlobalCounter+1;
		__WFI();
	}
}
